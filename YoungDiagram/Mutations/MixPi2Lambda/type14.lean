...
noncomputable def X14 : Mix (Pi, 2 • Lambda) := by
  ...
  rw [mem_Mix_iff, map_add, map_add, map_add, map_add, map_add, map_add,
    evenPart_ofRank_even (k := 2 * m + 2) (ε := ε) (by omega) ev_m,
    evenPart_ofRank_even (k := 2 * n + 2) (ε := -ε) (by omega) ev_n,
    oddPart_ofRank_even (k := 2 * m + 2) (ε := ε) ev_m,
    oddPart_ofRank_even (k := 2 * n + 2) (ε := -ε) ev_n,
