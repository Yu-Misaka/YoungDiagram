...
inductive Primitive : Mix (Lambda, Pi) → Mix (Lambda, Pi) → Prop
...
inductive Step : Mix (Lambda, Pi) → Mix (Lambda, Pi) → Prop
  | mk (X Y Z : Mix (Lambda, Pi)) (h : Primitive X Y) :
      Step (X + Z) (Y + Z)

lemma Primitive.isMutation {X Y : Mix (Lambda, Pi)} (h : Primitive X Y) :
    IsMutation X Y := by
  cases h with
  | type4 ε hε h_le =>
...
lemma Primitive.neg {X Y : Mix (Lambda, Pi)} (h : Primitive X Y) :
    Primitive (- X) (- Y) := by
  cases h with
  | type4 ε hε h_le =>
...
lemma Step.isMutation {X Y : Mix (Lambda, Pi)} (h : Step X Y) :
    IsMutation X Y := by
  cases h with
  | mk X Y Z h =>
    exact .add_right _ (Primitive.isMutation h)

lemma Step.neg {X Y : Mix (Lambda, Pi)} (h : Step X Y) : Step (- X) (- Y) := by
  cases h with
  | mk X Y Z hPrime =>
    rw [Mix.Lambda_Pi_neg_add, Mix.Lambda_Pi_neg_add]
    exact Step.mk (- X) (- Y) (- Z) hPrime.neg

lemma Step.add_right (W : Mix (Lambda, Pi)) {A B : Mix (Lambda, Pi)}
...
lemma Step.of_neg {X Y : Mix (Lambda, Pi)} (h : Step (-X) (-Y)) : Step X Y := by
